<template>
  ...
  >
    <output
      ref="output"
      class="inline-block form-select cursor-pointer w-full relative whitespace-no-wrap"
        ...
        <slot name="output" :selected="selected">
          <template v-for="i in selected">
            <template v-if="multiple">
              <div
                :key="i.key"
                class="inline-block border bg-gray-200 pl-1 pr-6 mr-1 rounded relative"
              >
...
export default defineComponent({
  ...
  setup: (props, ctx) => {
    ...
    const offsetHighlight = (offset: number): void => {
      ...
      let index = entries.findIndex((i) => i.key === highlight.value);
      index += offset;
      if (index < 0) {
        index = 0;
      }
